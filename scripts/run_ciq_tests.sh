#!/usr/bin/env bash
# Launch the Connect IQ simulator headlessly, wait until it actually accepts IPC,
# run the compiled --unit-test PRG under a HARD timeout, then hand the captured
# output to the strict fail-closed parser. The readiness probe (not a `sleep`)
# and the `timeout` are what make this fail-fast instead of hanging to the job
# limit the way the image's own tester.sh does (issue #42; observed on PR #40).
set -euo pipefail

PRG="${1:?usage: run_ciq_tests.sh <test.prg> <device>}"
DEVICE="${2:?usage: run_ciq_tests.sh <test.prg> <device>}"
SIM_PORT="${CIQ_SIM_PORT:-1234}"        # PINNED from `ss -ltnp` (DIAGNOSTIC step)
...
# `connectiq` may be a launcher that forks the real "$SIM_PROC" and returns, so
