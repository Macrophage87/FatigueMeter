...
**How to read the citations.** Each load-bearing claim is followed by a source. Verification flags used throughout: **[confirmed]** = adversarially fact-checked or corroborated across ≥2 primary sources; **[partial]** = supported but with a caveat noted; **[unverified]** = paywalled or otherwise not fully read; **[synthesis]** = the author's engineering inference, not a result reproduced from a source. A consolidated bibliography with URLs is in [references.md](references.md).
...
The decisive implementation lesson across all sources: **the slow exponential's time constant τ₂ is not identifiable.** Barstow's τ₂ point estimates ranged from ~180 s to 6.7×10⁵ s; Bell et al. (2001) found τ and slow-amplitude estimates differed significantly with model/window choice and could trade off against the delay parameter **[Bell: partial — paywalled, conclusions from abstract]**. Over a finite bout the slow rise is effectively **linear**. Bell's practical guidance: for heavy exercise, a three-component model over the full 6 min, or a two-component model fitted from 20 s onward, fits best; onset of the slow component estimated via a phase-3 delay is **~2 min, earlier than the arbitrary 3 min** often assumed **[confirmed]**.
...
```
(Barsumyan et al. 2026; frai.2025.1623384) **[confirmed, verbatim equations]**. The industry-standard TrainingPeaks/Coggan variant substitutes **Efficiency Factor EF = Normalized Power / average HR** per half; note that the exact `EF = NP/avgHR` definition is standard Coggan doctrine but was **not** stated verbatim in the fetched primary sources **[partial]**.
...
**Corroborating channel — cadence decline.** Barsumyan et al. (2026, *BMC Sports Sci Med Rehabil*) found second-half cadence decline is significantly associated with both drift metrics (mixed-model b = 0.61, p = 0.024): **each 1 rpm of cadence decline ≈ +0.61% cardiovascular drift and ≈ +0.58% aerobic decoupling** **[partial — full text not read, from abstract]**. Cadence is free in the FIT stream and lags neuromuscular fatigue, so it is a useful second vote.
...
- **Cycling time-to-task-failure** (Rogers, Fleitas-Paniagua, Murias et al. 2025, at 95% RCP, n=10): metabolic responses were stable over Q2–Q4 while HR, DFA-α1, and respiratory frequency all drifted to task failure, with strong session-to-session repeatability (ICC 0.73–0.94) **[partial — Springer full text paywalled; pattern + repeatability confirmed, exact per-quarter α1 values not obtained]**.
...
**Concerning-state heuristics (not formally validated):** 0.75 = leaving the purely aerobic domain; ~0.5 = at/near the highest sustainable intensity; **<0.5 at an intensity the athlete could hold when fresh, or a drop of ≳0.3 below their own baseline for that power, is a defensible fatigue flag** **[synthesis — the alarming empirical anchor is that fatigued athletes reach 0.32–0.37 at low aerobic external loads]**.
...
- **Rothschild et al. 2025 durability prediction** (51 cyclists, EJAP). Predicts power at VT1 *after* ~2–2.5 h of riding from pre/post decoupling: best GEE model `Post-VT1 ≈ 21.7 + 1.0·(Baseline VT1) − 0.58·ΔFR − 0.003·(ΔHR·Time) − 0.53·VO₂peak`, **R²=0.95 (0.93 bootstrap), MAE=7.2 W**; strongest single predictor is **HR decoupling (r_rm = −0.76)** **[confirmed]**. **Caveat:** the full model needs respiratory frequency (ΔFR) and VO₂peak, so it is not directly implementable on a power+HR+RR device — but the transferable insight is that **HR decoupling at a fixed power is the dominant, cheapest durability marker**, and it is a pre/post test-battery model rather than a within-ride estimator.
