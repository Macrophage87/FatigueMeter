...
- **Feat vs Attrition classification (white-paper §8.2):** on a synthetic **hard-effort ride** (repeated supra-CP surges with maintained/rising power, W′ depletions, best efforts), high fatigue is classified **Feat of Strength** and the verdict is **not** "turn back." On a synthetic **long grind** (sub-threshold power, rising decoupling and α1 drift, past the durability anchor, output flat/falling), high fatigue is classified **Attrition** and the turn-back verdict fires. Assert FeatScore dominates in the first and AttritionScore in the second; assert a W′ "match" is counted only on a genuine W′bal <threshold→recovery cycle.
