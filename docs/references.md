...
- ✅/🟡 Rogers, Fleitas-Paniagua, Trpcic, Zagatto, Murias (2025), durability/TTF, *EJAP* 125(6):1619–1631, PMID 39904800. — https://doi.org/10.1007/s00421-025-05716-2 — **verified via the lead author's full-text reproduction** at http://www.muscleoxygentraining.com/2025/02/dfa-a1-respiratory-rate-as-measures-of.html. Cycling TTF at 95% RCP/MMSS, n=10 (5M/5F), TTF ~46 min (Control 46.6±18.9 vs Reward 45.3±21.3, p=0.54). HR, DFA-α1, fB all drift Rest→Q4 while VO₂/lactate/glucose stay flat over Q2–Q4. **RM-ANOVA:** DFA-α1 F=29.06, η²=0.63; HR F=369, η²=0.95; fB F=58, η²=0.76 (all p<0.001). Repeatability — **abstract summary band ICC 0.73–0.94, r 0.83–0.98** (this is the source-of-record figure; an earlier draft mis-stated the ceiling as 0.96/0.97 from wider individual Table-2 cells). Per-quarter DFA-α1 pattern ~1.2 (Q1) → ~0.75 (Q4) is **figure-read** (exact supplementary-table means still paywalled). **Evidence strength:** n=10, single group; ICC lower bound 0.73 ("moderate"); η²=0.63 upward-biased at this n. **Key nuance:** not all participants reached anticorrelated (<0.5) values even at failure — failure occurred across a range of personal α1/fB values; α1 intensity thresholds lose validity once fatigued. **This is the *only cycling* durability study; the fatigue-quantification use otherwise rests on running (ultra/marathon).**
...
- ✅ *From Lab to Wrist* (arXiv 2505.00101) — **⚠ non-peer-reviewed preprint.** https://arxiv.org/html/2505.00101 — neural Kalman HR/VO₂; level+velocity HR sub-model; HR MAE 2.81 bpm. Borrowed as *architecture only*; no power, no HRV.
- 🟡 *PM-EKF* (arXiv 2604.26803) — **⚠ non-peer-reviewed preprint; internals unverifiable.** https://arxiv.org/html/2604.26803 — 5-state EKF gas exchange; median R²=0.72. *(Q/R covariances & Jacobians supplementary-only — unread; HR contributed no significant accuracy in their setup.)* Borrowed as *filter architecture only* — its result does not transfer.
...
- 🟡 Banister TRIMP. — https://www.trainingimpulse.com/banisters-trimp-0 · https://www.veohtu.com/trimp.html — *(female coefficient **0.86 vs 0.64** discrepancy across secondary sources — a ~34% ambiguity in a coded constant. **Must be resolved against Banister 1991 primary before implementation, or exposed as a user setting**; the men's `0.64·e^(1.92x)` sharing 0.64 with one reported female value signals a transcription error somewhere upstream. This is a defect, not merely a flag.)*
