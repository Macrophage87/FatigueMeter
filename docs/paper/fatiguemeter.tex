...
\author{%
  \normalsize [Author Name(s)]\\[2pt]
  \small\itshape [Affiliation]\\
  \small\texttt{[correspondence@example.org]}%
}
\date{\today}
...
\text{AttritionScore} &\propto (\text{decoupling above baseline})\cdot t_{\text{sub-threshold, past anchor}} + (\aone\ \text{drift below baseline}),
\label{eq:scores}
\end{align}
