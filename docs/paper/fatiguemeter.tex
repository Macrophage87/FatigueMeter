...
The system is organised as three coupled layers (\autoref{fig:arch}). \textbf{Layer~1} computes cheap, directly measured quantities --- the validated backbone. \textbf{Layer~2} fuses them into a latent acute-fatigue index that is calibrated per athlete. \textbf{Layer~3} accounts for the slow, residual fatigue that the athlete brings to a ride and carries away from it. The layers were originally coupled both ways --- Layer~3 seeding Layer~2's initial fatigue state and Layer~2's final state updating Layer~3. As a data field cannot maintain an honest cross-ride Layer-3 state, this coupling is broken on-device (\autoref{sec:glance}): $\Fstate(0)$ starts neutral, and the only Layer-3 output retained is the per-ride load (TSS), exported for the platform to accumulate (\autoref{sec:startend}).
...
$g_P$ & $\approx (HR_{\max}-HR_{\text{rest}})/P_{\text{at }HR_{\max}}\approx 0.45$\,bpm/W & static gain (denominator is power at $HR_{\max}$, $\approx$ threshold; an earlier $\approx0.15$ took a sprint-peak $P_{\max}$ and saturated the index) \\
$P_{\text{AeT}}$, CP & from athlete ($P_{\text{AeT}}\approx 0.75\,$FTP) & profile; a stale value propagates downstream \\
$a_0,a_1,s$ & $1.0,\;0.5,\;0.02\,\text{W}^{-1}$ & sigmoid crossing $\aone=0.75$ at $P_{\text{AeT}}$ (midpoint $a_0-a_1/2=0.75$; asymptotes $1.0$ rest, $0.5$ floor) \\
...
\label{eq:seed}
\end{equation}
so that a rider deep in negative TSB with suppressed RMSSD starts partly fatigued. Because $f(\cdot)$ is unvalidated, start-of-ride fatigue was presented as a coarse bucket, never a point value. \emph{On-device this map is no longer evaluated} (\autoref{sec:glance}): it depends on cross-ride ATL/TSB/RMSSD state a data field cannot hold honestly --- a blind ledger sits at $\text{TSB}\approx0$, collapsing $f(\cdot)$ to $\Fstate(0)\approx0$ for everyone, including a genuinely fatigued rider. The watch therefore starts $\Fstate(0)$ neutral and makes no pre-ride fatigue claim; the map remains documented for a platform or companion that holds the complete residual state.
...
The layout, top to bottom, is: (1) a \textbf{status band} in descriptive states --- \textsc{fresh/productive} (green), \textsc{fatigue building} (amber), \textsc{durability markers drifting} (red) --- with, when red, a second line naming the kind of red (Feat of Strength or Attrition) as characterisation, not a command; (2) an \textbf{acute-fatigue dial}; (3) an \textbf{evidence row} (rolling decoupling, $\aone$ with a data-quality dot, intensity-weighted kilojoules versus the durability anchor, and $W'$ matches burned); (4) a \textbf{feats strip} of the ride's best efforts --- peak 1-, 5-, and 20-minute power, all directly measured this ride; and (5) a \textbf{data-quality footer} (RR artifact percentage and $\aone$ validity, with the decoupling-only fallback surfaced when RR is poor). Colour is always reinforced with text or icon, never colour alone, for colour-blind riders and glance speed.

A recorded design decision governs the dial. A number on a dial reads as a measurement regardless of surrounding tags, and a weakly observable, prior-dominated, externally unvalidated fatigue number should not be presented as one. \emph{The precise 0--100 \afi{} digit, the point-value start/current/end figures, and any projected-end tick are gated on a positive external criterion study} (\autoref{sec:validation}). Pre-study, the dial shows only the three-state green/amber/red categorical plus the coarse current position; the Feat/Attrition characterisation and the validated backbone (decoupling, kilojoules versus anchor, the population $\aone$ anchor) remain fully available. Shipping the numeric \afi{} earlier is a legitimate product choice but must be recorded as an explicit exception to this decision.

A second recorded decision governs the training-scale layer. The residual CTL/ATL/TSB and start-of-ride form, and the dial's start marker, are \emph{not} surfaced on the in-ride glance. A watch data field accumulates load only from the rides it actually runs --- it never sees other-app rides, runs, indoor sessions, or a morning resting-HRV reading --- so an in-ride TSB or start-form readout drifts and cannot be kept honest without the rider manually re-seeding it before each ride, upkeep only long rides and races justify; that record belongs to the training-load platform, not to a per-ride tile. A subsequent decision extends this from the glance to every surface the field cannot back. Hiding the untrustworthy cross-ride state while it still \emph{influenced} the acute path and the export was only half the fix, so the influence is removed: $\Fstate(0)$ now starts \textbf{neutral} and no longer reads the ledger (a blind ledger sits at $\text{TSB}\approx0$, so seeding from it silently gave a fatigued rider $\Fstate(0)\approx0$ --- a hidden wrong worse than the tile that was removed); the on-device app \textbf{no longer computes, persists, or exports cross-ride CTL/ATL/TSB}, the ledger being reduced to the one honestly computable per-ride quantity, ride load (TSS, or HR-TRIMP fallback), exported per-ride to the FIT for the platform to accumulate; and the session summary drops start- and added-fatigue, keeping ride-induced drift (acute $\Fstate$ from the neutral start), TSS, peak \afi{}, the red split, and durability kilojoules --- all ride-measured. The seeding map~\eqref{eq:seed} and the cross-ride EWMAs~\eqref{eq:ctl} remain documented as the platform's role; they are not run on-device.
...
Two tiers persist the markers through the ride and roll them up for cross-ride comparison. \emph{In-ride time series} are logged (1\,Hz, or 5\,s for $\aone$) as developer fields --- \afi{}, $\Fstate$, decoupling, $\aone$, $W'$ balance, intensity-weighted kilojoules, and the two effort scores --- so they flow into the activity file for post-ride charting. A \emph{session summary} persists a compact record for cross-ride comparison: date, duration, TSS, ride-induced drift (acute $\Fstate$ from the neutral start), peak \afi{}, time-in-red split into Feat versus Attrition minutes, FeatScore with top feats, AttritionScore, and durability anchor reached, with a rolling history retained. (Start- and added-fatigue and cross-ride CTL/ATL/TSB are deliberately \emph{not} recorded or exported: the field cannot compute them honestly, so only the per-ride TSS leaves the device, for the platform to accumulate.) A post-ride comparison view lets the rider see whether hard days are bought with performance or paid for with drift.
...
% ==================================================================
\appendix
\section{Modelled behaviour of each fatigue variable}\label{sec:figures}
% ==================================================================

The figures below model how each fatigue variable behaves \emph{as fatigue increases}, computed from the model equations of \autoref{sec:layer1}--\autoref{sec:layer3} with the seed constants of \autoref{sec:tuning}. For the acute observables the abscissa is the latent drift state $\Fstate$; for the accumulators it is time on task or training days. They are generated by a self-contained script (\texttt{docs/figures/generate\_figures.py}) and provided in vector (\texttt{.pdf}/\texttt{.svg}) and raster (\texttt{.png}) form.

\emph{Read these as shapes, not measurements.} The fused \afi{} and the durability advisory are synthesis-grade and unvalidated against any external fatigue criterion (\autoref{sec:validation}); the constants are per-athlete-calibratable defaults. Colour is always reinforced with text or line style.

\begin{figure}[htbp]\centering
\includegraphics[width=\linewidth]{00_overview}
\caption{Overview: modelled behaviour of each fatigue variable with increasing fatigue.}
\label{fig:overview}
\end{figure}

\begin{figure}[htbp]\centering
\includegraphics[width=0.72\linewidth]{01_F_drift_state}\\[2pt]
\includegraphics[width=0.72\linewidth]{02_AFI_index}
\caption{\textbf{Layer 2.} The residual cardiovascular-drift state $\Fstate$ charges toward $F_{\text{ss}}=\text{charge}\cdot\tau_{\text{rec}}$ during work and \emph{relaxes} on recovery ($\kappa_d$ active-gated); the Acute Fatigue Index $\afi=100\,\mathrm{clamp}(\Fstate/F_{\text{ref}})$ climbs through the descriptive bands. \afi{} is an index, not a measurement.}
\label{fig:layer2}
\end{figure}

\begin{figure}[htbp]\centering
\includegraphics[width=\linewidth]{03_DFA_alpha1}
\caption{\textbf{DFA-$\alpha_1$.} (a) the power$\to\alpha_1$ map crosses the $0.75$ aerobic-threshold anchor at $P_{\text{AeT}}$; $0.50$ is the $P\!\to\!\infty$ lower asymptote, not a finite threshold. (b) at fixed power, fatigue pulls $\alpha_1$ below its baseline-for-power via the $-c_F\Fstate$ coupling --- the drift, not the absolute value, is the signal.}
\label{fig:alpha1}
\end{figure}

\begin{figure}[htbp]\centering
\includegraphics[width=0.49\linewidth]{04_decoupling}\hfill
\includegraphics[width=0.49\linewidth]{05_efficiency_factor}\\[2pt]
\includegraphics[width=0.49\linewidth]{06_hr_drift}\hfill
\includegraphics[width=0.49\linewidth]{07_cadence_drift}
\caption{\textbf{Layer 1 primitives.} Aerobic decoupling rises and Efficiency Factor falls as drift lifts HR at fixed power; heart rate at fixed power is $HR_{ss}+\Fstate$; cadence decline is a \emph{weak} corroborator ($r\!\approx\!0.40$, $\sim$16\% variance, direction-ambiguous) shown as a low-confidence cloud, not a law.}
\label{fig:primitives}
\end{figure}

\begin{figure}[htbp]\centering
\includegraphics[width=0.49\linewidth]{08_wprime_bal}\hfill
\includegraphics[width=0.49\linewidth]{09_kj_durability_clock}
\caption{\textbf{Anaerobic reserve and durability clock.} $W'$balance depletes in a sawtooth over hard intervals (dips below 20\% then recovery are ``matches''); intensity-weighted kilojoules accumulate toward the durability anchor and diverge from raw volume only on supra-CP surges, where the weighting is active.}
\label{fig:wbal_kj}
\end{figure}

\begin{figure}[htbp]\centering
\includegraphics[width=0.49\linewidth]{10_ctl_atl_tsb}\hfill
\includegraphics[width=0.49\linewidth]{11_rmssd_baseline}\\[2pt]
\includegraphics[width=0.62\linewidth]{12_feat_vs_attrition}
\caption{\textbf{Residual load and effort character.} Under a build block ATL leads CTL so TSB $=$ CTL$-$ATL goes negative; resting RMSSD declines below the personal $-1$\,SD band under sustained overreaching; FeatScore (output-bought) and AttritionScore (drift-bought) characterise, but never gate, high-fatigue ``red''.}
\label{fig:residual}
\end{figure}
