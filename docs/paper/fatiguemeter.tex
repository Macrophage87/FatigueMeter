...
A second recorded decision governs the training-scale layer. The residual CTL/ATL/TSB and start-of-ride form, and the dial's start marker, are \emph{not} surfaced on the in-ride glance. A watch data field accumulates load only from the rides it actually runs --- it never sees other-app rides, runs, indoor sessions, or a morning resting-HRV reading --- so an in-ride TSB or start-form readout drifts and cannot be kept honest without the rider manually re-seeding it before each ride, upkeep only long rides and races justify; that record belongs to the training-load platform, not to a per-ride tile. Layer~3 is retained internally: it still folds silently into the persistent ledger, seeds $\Fstate(0)$ (defaulting to neutral form), and is written to the post-ride FIT summary (start, end, and added fatigue; TSS; CTL/ATL) for later analysis --- it is simply no longer shown as a live glance readout the field cannot back.
