...
Layer 1 computes cheap, directly-measured quantities — these are the **validated backbone**. Layer 2 fuses them into a latent acute-fatigue *index* (synthesis-grade, calibrated per athlete). Layer 3 accounts for the slow, residual fatigue that the athlete brings to the ride and carries away from it. The layers were originally coupled both ways (Layer 3 seeding Layer 2's F(0), Layer 2's end-state updating Layer 3). **Rev 5** breaks the on-device coupling: a data field can't hold an honest cross-ride Layer-3 state, so F(0) starts **neutral** and the only Layer-3 output kept is the **per-ride load (TSS)**, exported for the platform to accumulate. Start-of-ride residual fatigue is not claimed on-device.
...
Seed HR(0), A1(0) from the first valid measurements; F(0) starts neutral (= 0) — not seeded from Layer 3 (Rev 5, see §7).
...
- **Start-of-ride** fatigue = Layer 3 state at ride start. The seeding map `F(0) = f(ATL, TSB, RMSSD_deviation)` converts a days-scale residual-load state into "bpm of pre-existing drift" — a **cross-domain mapping with no cited basis (synthesis-grade, §9).** A concrete default was: `F(0) = F_ref · clamp( a·max(0,−TSB)/TSB_scale + b·max(0,−RMSSD_z) , 0, 0.6 )`, with `a,b,TSB_scale` configurable. **Because `f()` is unvalidated, start-of-ride fatigue was presented as a COARSE BUCKET, never a point value.**
  > **Rev 5 — F(0) starts neutral on-device.** This map depends on cross-ride ATL/TSB/RMSSD state a data field cannot maintain honestly (§8.1 Rev 5): a blind ledger sits at TSB≈0, so `f()` collapses to F(0)≈0 for *everyone*, including a genuinely fatigued rider — a silent wrong. The on-device app therefore **starts F(0) neutral (= 0)** and does not evaluate `f()`. The map remains documented for a platform/companion that *does* hold the complete residual state and could supply a start-form input; on the watch, pre-ride fatigue is simply not claimed.
...
2. **Acute Fatigue dial.** *Pre-pilot (default — see §8.1 decision below):* the dial shows the **3-state green/amber/red band** and the **now** position **coarsely** (no precise 0–100 digit, no projected tick). *Post-pilot:* the precise **AFI 0–100** digit unlocks, with a **now** tick and a **projected end-of-ride** rendered as a **shaded "model projection" range** (never a hard tick equal in authority to "now"). The (bucketed) **ride-induced drift** over the ride is reported in the **post-ride summary**, not as an in-ride dial marker (Rev 4). Pre-ride/start-of-ride fatigue is no longer estimated on-device — F(0) starts neutral (Rev 5).
...
> **Rev 4 — training-scale (Layer 3) context is off the in-ride glance.** TSB / start-fatigue and the dial's start marker were **removed from the glance screen**. Rationale: a watch data field only accumulates load from the rides it actually runs — it never sees other-app rides, runs, indoor sessions, or a morning resting-HRV reading (the resting-RMSSD baseline has no on-device source) — so an *in-ride* CTL/ATL/TSB or start-form readout drifts and can't be kept honest without the rider manually re-seeding it before each ride, upkeep that only long rides/races justify. That record properly lives in the training-load platform (e.g. intervals.icu / Training Status), not in a per-ride tile.

> **Rev 5 — remove Layer 3 from every surface the field can't back, not just the glance.** Hiding the untrustworthy cross-ride state while it still *influenced* the acute path and the export was only half the fix, so Rev 5 removes the influence: **(a)** F(0) now starts **neutral** and no longer reads the ledger (a blind ledger sits at TSB≈0, so seeding from it silently gave a fatigued rider F(0)≈0 — a hidden wrong worse than the tile that was removed); **(b)** the on-device app **no longer computes, persists, or exports cross-ride CTL/ATL/TSB** — the ledger is reduced to the one honestly-computable per-ride quantity, **ride load (TSS, or HR-TRIMP fallback)**, which is exported per-ride to the FIT where the platform does the complete chronic accumulation; **(c)** the FIT/session summary drops `start_fatigue`/`fatigue_added` and keeps **ride-induced drift** (acute `F` from the neutral start), TSS, peak AFI, the red split, and durability kJ — all ride-measured. The `f(ATL,TSB,RMSSD)` seeding map (§7) and cross-ride EWMAs (§5) remain **documented** as the platform's job; they are no longer run on-device. The now-vestigial `ctlSeed`/`atlSeed`/`seedA`/`seedB`/`seedTsbScale`/`tsbFresh`/`tsbOverreach`/`acwrEnabled` settings (and the `CTL_TAU`/`ATL_TAU`/`TSB_FRESH`/`TSB_OVERREACH` constants) have been **removed** from `Config`, `properties.xml`, `settings.xml`, and `strings.xml`.
...
**(b) Session summary → FIT session fields + persistent app storage.** At ride end, write `MESG_TYPE_SESSION` developer fields **and** persist a compact **Session Result** for cross-ride comparison: date, duration, TSS, **ride-induced drift** (acute `F` from the neutral start), peak AFI, **time-in-red split into Feat minutes vs Attrition minutes**, FeatScore + top feats (best 1/5/20-min power, matches burned), AttritionScore, and durability anchor reached (kJ). Keep a **rolling history** (e.g., last N sessions) in `Storage`. *(Rev 5: `start_fatigue`/`fatigue_added` and cross-ride CTL/ATL/TSB are no longer written — the field can't compute them honestly; only the per-ride TSS is exported, for the platform to accumulate.)*
...
#### 8.4.1 The `initialize()` contract and the catchability boundary (init-path constructors)

The §8.4 graceful-degradation strategy rests on two guards: `initialize()` writes a renderable NODATA snapshot first (`FatigueMeterView.initialize()`), and `compute()`/`onUpdate()` are wrapped top-to-bottom (`FatigueMeterView.compute()` / `FatigueMeterView.onUpdate()`) so the field greys but never bricks to the Connect IQ "IQ…" banner. Those guards are only as strong as *what they can actually catch*. This sub-rule states the contract that keeps init-path constructors inside that reach. (The static tripwire for it — `scripts/check_init_contract.py`, rule R1 — is created under #116; see "Enforcement seams" below. This section is the load-bearing contract; the lint is the tripwire, not the contract.)

**The two-sided invariant.** A constructor or step reachable from `initialize()` (directly, or via the render-first one-shot `ensureBuilt()` on the compute path, `FatigueMeterView.ensureBuilt()`) must satisfy **both**:

- **(a) Must NOT run fallible/heavy work where a throw is uncatchable** — `Storage`/`Properties` I/O, cross-scope `method(:hidden)`-invoke chains, recovery/migration, OOM-risky allocation — i.e. anywhere a throw aborts construction *before the top-level §8.4 guard is live*, or that faults through one of the VM's non-throw channels below.
- **(b) Must NOT move an init-only platform API off `initialize()`** — `FitContributor.createField()` foremost — because calling it outside its legal lifecycle window is an uncatchable `System Error` no matter how many `try/catch` layers wrap it.

**Unifying rule:** `try/catch` + the `ready` flag defend **only the VM's *catchable-throw* channel.** Everything outside that channel — symbol-resolution aborts, out-of-phase platform calls, synchronous hangs, and throws fired before a guard exists — defeats §8.4 however the code is wrapped. The contract is: never place a fallible or lifecycle-contract-bound operation where its failure lands outside the catchable channel.

**Catchable, for contrast** (a normal `throw` inside a live `try/catch`, which §8.4 *does* handle): `Storage.get/set/deleteValue` throwing on quota/serialization (`source/SessionStore.mc:75`, `:184`, `:202`); `createField()` *rejecting a field* for missing FitContributor permission, swallowed per-field by `mkRec` (`source/FitLogger.mc:56-64`); ANT `GenericChannel.open()` *throwing*, caught in `AntHrm.start()` (`source/AntHrm.mc:74`); any calculator throw on the compute path, caught by `compute()`'s top-level try.

**The five-class catchability taxonomy** (each defeats a textually-wrapping `try/catch`):

1. **Symbol-Not-Found / "Failed invoking" from a cross-scope `method(:hidden)` invoke.** A bound reference to a `hidden` instance method fails to resolve when `.invoke()`-d from another (e.g. `static`) scope, aborting *at the invoke site — the target body never runs*. `persist()` passed `method(:tryWrite)` into the static `shedWrite()` (`source/SessionStore.mc:113`), invoked as `writer.invoke(work)` (`:140`); documented in full at `source/SessionStore.mc:164-182` ("try/catch cannot catch it"). This is **#109**'s proven exemplar (fix = un-hide `tryWrite`, `:183`).
2. **System Error / "Failed invoking" from an out-of-phase platform call.** `FitContributor.createField()` is init-only; render-first (#103/#108) had deferred `new FitLogger(self)` out of `initialize()` into `ensureBuilt()` (the first statement of `FatigueMeterView.computeInner()`), so `df.createField` (`source/FitLogger.mc:58`) fired on tick 1 and **bypassed all four** nested catches — `mkRec` and `FitLogger.initialize()` (`source/FitLogger.mc:56-64`, `:35-40`), plus `ensureBuilt()` and `compute()` (both in `FatigueMeterView`). Note the causation: **moving the guarded build off `initialize()` is what *created* this class of fault** — render-first cured the load-path hang but relocated an init-only API into an illegal phase. This is **#116**'s proven exemplar; **fixed under #116 / PR #117 (merged to `main`)** — `FitLogger` is now constructed eagerly in `initialize()`, so `createField()` runs in its legal window.
3. **Synchronous hang / never-returns.** `try/catch` traps throws, not non-termination — stated verbatim in `FatigueMeterView.ensureBuilt()`'s doc comment. The ANT `open()` (`source/AntHrm.mc:74`) is the #90/#104 hang suspect: a *throw* there is catchable, a *hang* is not — same call, different channel. Render-first only *relocates* it off the load path onto the watchdog-killable compute path; it never cures it.
4. **Construction abort before the top-level guard exists — a phase *axis*, not a distinct fault mechanism.** A throw during `initialize()` aborts the `DataField` ctor before `ready` is set and before any compute/onUpdate try-wrap is reachable, so `getInitialView()` fails and the NODATA baseline never paints (`source/SessionStore.mc:169-174`). This is *why* class 1 was fatal in the pre-render-first build: its symbol fault fired inside `initialize() → load() → reconcileActive() → persist()`. Read this as the **phase orthogonal to catchability**: the *same* fault is fatal in the init phase (no guard yet) and survivable on the compute path (guard live). Render-first shifts the phase for *catchable throws only* — it moves the guarded build to where a live guard wraps it. It does nothing for classes 1-3 and 5, whose faults are uncatchable in *either* phase; that orthogonality is precisely how #116 slipped through.
5. **API illegal for the current app type** (sibling of class 2 — right code, wrong context). `Sensor.registerSensorDataListener` from a `DataField` is an uncatchable crash (`FatigueMeterView.registerSensors()`, `source/AntHrm.mc:5-11`); the codebase avoids it by opening a raw ANT+ channel instead.

**OOM (excluded, noted for completeness).** Out-of-memory during construction is a resource-exhaustion channel, not a throw channel: it manifests as a watchdog trip / device reset, not a catchable exception, so no `try/catch` placement helps. It is **not** enumerated as a first-class member here because a construction OOM was specifically *refuted* for this app (render-first AC-1, noted in `FatigueMeterView.initialize()`'s comment). Rule (a)'s "OOM-risky allocation" clause keeps it in scope as a placement constraint even though it is not a catchability class.

**One-time init-path constructor audit.** Under render-first `initialize()` does cheap field assignment + `defaultSnapshot()` **plus the eager `new FitLogger(self)`** (kept init-only per #116) — clean (`FatigueMeterView.initialize()`). The *remaining* collaborator ctors moved into `ensureBuilt()` and `AntHrm` into `registerSensors()` (both in `FatigueMeterView`); audit each against **both** sides:

| Init-path ctor | Fallible / contract-bound op | Side | Status on `main` |
|---|---|---|---|
| `FitLogger` | `createField()` — **init-only platform API** | **(b)** | **Fixed (#116 / PR #117, on `main`)** — constructed **eagerly in `initialize()`**, so `createField()` runs in its legal window. *Historical:* #108 render-first had deferred it onto the compute path via `ensureBuilt()` — the class-2 fault (#116) |
| `SessionStore` | Storage reads (`load()`) + `reconcileActive() → persist()` **write**; historically the class-1 `method(:hidden)` invoke | **(a)** | Throw is now **catchable** (build runs under `ensureBuilt()` + `compute()` guards); the class-1 fault was fixed by un-hiding `tryWrite` (#109). Relocating the recovery *write* out of the ctor into an explicit `recover()` stays **#112/#103** |
| `Config` | `Properties.getValue` reads in `reload()` | (a) | **Clean** — every read is individually `try/catch`-guarded and null-coalesced to a default (`source/Config.mc:106-131`); no uncatchable init work. Allocates nothing notable |
| `PrimitivesCalculator` | **none** — does **no** `Property`/`Storage` I/O | — | **Clean, allocation-only.** Stores `cfg` and allocates four `RingBuffer`s (`source/PrimitivesCalculator.mc:44-62`). *(Correction: earlier drafts listed it as doing "Property/Storage reads" — it does not; do not carry that claim.)* |
| `AcuteFatigueFilter` | none | — | Clean; **allocates** a 4×4 matrix + state array |
| `EffortCharacterizer` | none | — | Clean; **allocates** three `BestEffort` buffers |
| `TrainingLoadLedger` | none | — | Clean; pure field assignment |
| `AntHrm` | ANT `GenericChannel.open()` (deferred to `start()`, not the ctor) | (a)/class 3 | Ctor only configures the channel; the *open* is the class-3 hang suspect, relocated by render-first, not cured |

The audit's standing job: keep this table current, and enforce that a **(b)**-class op is never deferred off `initialize()` and an **(a)**-class op never sits in a pre-guard/uncatchable spot. "Clean, allocation-only" is called out explicitly because allocation is the OOM-risk surface rule (a) still governs, even where there is no I/O.

**Enforcement seams (references, not duplication).** This doctrine is the *contract*; the guards live in siblings so no issue blocks on this one:

- **#116** owns the required static gate in `scripts/check_init_contract.py` (created in **PR #117**) — **two** rules, both #116's: **R1** (constructor placement — no `new FitLogger` / init-only-API owner constructed outside `initialize()`) and **R2** (`.createField(` confined to `FitLogger.mc`). Both are deterministic and false-positive-free; R1 is the precise check that would have blocked #108 pre-merge (a per-call-site `.createField(`-reachability check would be intractable, since the call lexically lives in `mkRec` — hence the placement/confinement framing). This section documents their fault as catchability **class 2**; it does **not** create or modify that script or those rules.
- **#114** adds **rule R3** (the `method(:hidden)` cross-scope-invoke footgun from #109) into the *same* `check_init_contract.py`, so the rules compose in one harness. (R1/R2 are #116's; #114's is the third rule.) This section documents R3's fault as catchability **class 1**.
- **#113** is the **dynamic** complement: boot with poisoned storage and advance **≥ 1 compute tick** (a "reaches first paint" assertion passes the NODATA frame and misses tick-1 faults). Static analysis forbids the *nameable* bad signatures from being written; the dynamic net catches what static cannot prove — class-3 hangs (undecidable), real runtime symbol faults, and any (b)-violation that renders one frame then dies. Static = R1/R2 (#116) + R3 (#114) in `check_init_contract.py`; dynamic = #113's boot-tick. They divide cleanly and are referenced here, not re-proposed.
