...
The §8.4 graceful-degradation strategy rests on two guards: `initialize()` writes a renderable NODATA snapshot first (`source/FatigueMeterView.mc:127-160`), and `compute()`/`onUpdate()` are wrapped top-to-bottom (`:262-266`, `:506-508`) so the field greys but never bricks to the Connect IQ "IQ…" banner. Those guards are only as strong as *what they can actually catch*. This sub-rule states the contract that keeps init-path constructors inside that reach. (The static tripwire for it — `scripts/check_init_contract.py`, rule R1 — is created under #116; see "Enforcement seams" below. This section is the load-bearing contract; the lint is the tripwire, not the contract.)

**The two-sided invariant.** A constructor or step reachable from `initialize()` (directly, or via the render-first one-shot `ensureBuilt()` on the compute path, `source/FatigueMeterView.mc:198-217`) must satisfy **both**:
...
2. **System Error / "Failed invoking" from an out-of-phase platform call.** `FitContributor.createField()` is init-only; render-first (#103/#108) deferred `new FitLogger(self)` (`source/FatigueMeterView.mc:209`) onto the compute path (`ensureBuilt()` is the first line of `computeInner()`, `:278`), so `df.createField` (`source/FitLogger.mc:58`) fires on tick 1 and **bypasses all four** nested catches (`mkRec` `:57`, `FitLogger.initialize` `:38`, `ensureBuilt` `:213`, `compute` `:262`). Note the causation: **moving the guarded build off `initialize()` is what *created* this class of fault** — render-first cured the load-path hang but relocated an init-only API into an illegal phase. This is **#116**'s proven exemplar (fixed under #116 / PR #117).
3. **Synchronous hang / never-returns.** `try/catch` traps throws, not non-termination — stated verbatim at `source/FatigueMeterView.mc:189-193`. The ANT `open()` (`source/AntHrm.mc:74`) is the #90/#104 hang suspect: a *throw* there is catchable, a *hang* is not — same call, different channel. Render-first only *relocates* it off the load path onto the watchdog-killable compute path; it never cures it.
...
5. **API illegal for the current app type** (sibling of class 2 — right code, wrong context). `Sensor.registerSensorDataListener` from a `DataField` is an uncatchable crash (`source/FatigueMeterView.mc:230-231`, `source/AntHrm.mc:5-11`); the codebase avoids it by opening a raw ANT+ channel instead.

**OOM (excluded, noted for completeness).** Out-of-memory during construction is a resource-exhaustion channel, not a throw channel: it manifests as a watchdog trip / device reset, not a catchable exception, so no `try/catch` placement helps. It is **not** enumerated as a first-class member here because a construction OOM was specifically *refuted* for this app (render-first AC-1, `source/FatigueMeterView.mc:170-171`). Rule (a)'s "OOM-risky allocation" clause keeps it in scope as a placement constraint even though it is not a catchability class.

**One-time init-path constructor audit.** Under render-first `initialize()` itself does only cheap field assignment + `defaultSnapshot()` — clean (`source/FatigueMeterView.mc:123-178`). The collaborator ctors moved into `ensureBuilt()` (`:202-209`) and `AntHrm` into `registerSensors()` (`:229-237`); audit each against **both** sides:

| Init-path ctor | Fallible / contract-bound op | Side | Status on `main` |
|---|---|---|---|
| `FitLogger` | `createField()` — **init-only platform API** | **(b)** | **VIOLATION** — sits on the compute path via `ensureBuilt()`; fixed under **#116 / PR #117** |
...
- **#116** owns the required static gate: **rule R1** in `scripts/check_init_contract.py` (created in **PR #117**), re-scoped to **constructor placement** — no `new FitLogger` / init-only-API owner constructed outside `initialize()` — which is deterministic and false-positive-free (a `.createField(`-reachability check is intractable because the call lexically lives in `mkRec`). R1 is the precise check that would have blocked #108 pre-merge. This section documents R1's fault as catchability **class 2**; it does **not** create or modify that script or rule.
- **#114** adds **rule R2** (the `method(:hidden)` cross-scope-invoke footgun from #109) into the *same* `check_init_contract.py`, so the rules compose in one harness. This section documents R2's fault as catchability **class 1**.
- **#113** is the **dynamic** complement: boot with poisoned storage and advance **≥ 1 compute tick** (a "reaches first paint" assertion passes the NODATA frame and misses tick-1 faults). Static analysis forbids the *nameable* bad signatures from being written; the dynamic net catches what static cannot prove — class-3 hangs (undecidable), real runtime symbol faults, and any (b)-violation that renders one frame then dies. Static = R1/R2 in `check_init_contract.py`; dynamic = #113's boot-tick. They divide cleanly and are referenced here, not re-proposed.
